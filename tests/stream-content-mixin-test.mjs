...
class TestEntry extends StreamContentEntryMixin(ContentEntry) {
    async getWriteStream(options) {
        return this._stream = (this.name === 'write_failure' ? new AlwaysFailingTestWritable() : new TestWritable());
    }
...
test("string write failure", async t => {
    ...
    await t.throwsAsync(async () => { return entry.setString('XXX'); }, { instanceOf: Error, message: 'write failed' });
});

test("buffer write failure", async t => {
    const entry = new TestEntry("write_failure");
    await t.throwsAsync(async () => { return entry.setBuffer(Buffer.from('abc')); }, { instanceOf: Error, message: 'write failed' });
});
